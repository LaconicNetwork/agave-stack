...
http_code=$(echo "$response" | sed '2q;d')
time_total=$(echo "$response" | sed '3q;d')

latency_ms="$(awk -v t="$time_total" 'BEGIN { print (t * 1000) }')"
...
echo "rpc_latency,endpoint=direct,method=getSlot latency_ms=${latency_ms},success=${success},http_code=${http_code}i,slot=${slot}i"
